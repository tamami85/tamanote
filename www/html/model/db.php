...
//insert文とupdate文でデータベースを書き込みするで関数
...
//メモを登録
function add_contents($db, $title, $contents, $date){
  $sql = "
          INSERT INTO
            memo(
              title,
          ...
          VALUES
            (?, ?, ?, ?)
        ";
  $params = array($title, $contents, $date, $date);
  return execute_query($db, $sql, $params);
}


//メモデータ取り出し
function select_contents($db){
  $sql = "
          SELECT
            id,  
            title,
...
//メモデータのアップデート
function update_memo($db, $title, $contents, $date, $id){
  $sql = "
          UPDATE
            memo
          SET
          ...
          WHERE
            id = ?
          ";
  $params = array($title, $contents, $date, $id);
  return execute_query($db, $sql, $params);
}